...
func (l *SyncOnecClusterLogic) SyncOnecCluster(in *pb.SyncOnecClusterReq) (*pb.SyncOnecClusterResp, error) {
	...
	client, err := l.svcCtx.OnecClient.GetOrCreateOnecK8sClient(l.ctx, in.Id, utils.NewRestConfig(cluster.Host, cluster.Token, utils.IntToBool(cluster.SkipInsecure)))
	if err != nil {
	...
	err = l.updateClusterIfChanged(cluster, clusterInfo, in.UpdateBy)
	if err != nil {
		return nil, err
	}
	...
	nodeModeList, err := l.svcCtx.NodeModel.SearchNoPage(l.ctx, "create_time", false, "cluster_id = ?", in.Id)
	if err != nil {
		l.Logger.Errorf("获取节点信息失败: %v", err)
		return nil, code.GetNodeInfoErr
	}
	...
	for _, dbNode := range nodeModeList {
		if _, exists := currentNodeMap[dbNode.NodeUid]; !exists {
			...
			if err := l.svcCtx.NodeModel.Update(l.ctx, dbNode); err != nil {
				...
				l.ChangeCLusterStatusFalse(cluster, in.UpdateBy)
	...
	for _, node := range currentNodeMap {
		...
		if exists {
			...
			nodeModel.Id = nodeId
			nodeModel.ClusterId = cluster.Id
			nodeModel.CreateBy = in.UpdateBy
			nodeModel.UpdateBy = in.UpdateBy
...
func (l *SyncOnecClusterLogic) updateClusterIfChanged(cluster *model.OnecCluster, clusterInfo *core.ClusterInfo, updateBy string) error {
	// 判断集群版本信息是否发生变化
	...
	// 判断构建时间是否发生变化
	if cluster.VersionBuildTime != clusterInfo.BuildTime {
		cluster.VersionBuildTime = clusterInfo.BuildTime
	}
	// 判断集群创建时间是否发生变化
	if !cluster.ClusterCreateTime.Equal(clusterInfo.CreateTime) {
		cluster.ClusterCreateTime = clusterInfo.CreateTime
	}

	// 更新修改人
	if cluster.UpdateBy != updateBy {
		cluster.UpdateBy = updateBy
	}
...
// 修改集群状态为false
func (l *SyncOnecClusterLogic) ChangeCLusterStatusFalse(cluster *model.OnecCluster, updateBy string) {
...
func CompareNodes(existingNode *model.OnecNode, node *core.NodeInfo) (*model.OnecNode, bool) {
	...
	// 比较并更新 JoinTime
	if !existingNode.JoinTime.Equal(node.JoinTime) {
		existingNode.JoinTime = node.JoinTime
		flag = true
	}
